...
const DEMO_LOADED_KEY = 'workload-pulse-demo-loaded';
const CAPACITY = 30;
...
function App() {
  ...
  const [demoLoaded, setDemoLoaded] =
    useState(
      () =>
        localStorage.getItem(
          DEMO_LOADED_KEY
  ...
  const loadTasks = async () => {
    ...
    try {
      ...
      const cleaned = normalized.filter((task) => {
        if (!demoTitles.has(task.title)) return true;
        if (seenDemoTitles.has(task.title)) return false;
...
const filteredTasks =
  ...
  const loadDemo = async () => {
   if (submitting || demoLoaded) return;
    ...
    try {
      ...
      for (const demoTask of demoSeed) {
        const matches = serverTasks.filter(
        ...
        if (existing) {
          const updated = normalizeTask(
            await taskApi.update(
              existing.id,
              toApiTask({ ...existing, ...demoTask })
            )
          );
          updatedDemoTasks.push(updated);

          // Remove older duplicate demo records from MockAPI.
          for (const duplicate of matches.slice(1)) {
            try {
              await taskApi.remove(duplicate.id);
            } catch {
              // Keep going if an old duplicate is already gone.
            }
          }
        } else {
...
const resetDemo = async () => {
  ...
  // Get demo tasks directly from the current ledger.
  const demoTasks = tasks.filter((task) =>
    demoTitles.has(task.title)
  );

  if (demoTasks.length === 0) {
    setDemoLoaded(false);
    setError('No demo data to reset.');
    setSubmitting(false);
    return;
  }

  // Remove demo tasks from the UI immediately.
  setTasks((current) =>
    current.filter(
      (task) => !demoTitles.has(task.title)
    )
  ...
  try {
    // Delete the same demo tasks from MockAPI.
    await Promise.all(
      demoTasks.map((task) =>
        taskApi.remove(task.id)
      )
    );

    // Allow Load Demo again only after reset succeeds.
    setDemoLoaded(false);

  } catch (err) {
    // Restore tasks if the API deletion fails.
    setTasks((current) => [
      ...demoTasks,
      ...current,
    ]);
    ...
    setError(
      `Demo data could not be fully reset. ${err.message}`
    );
  } finally {
    setSubmitting(false);
  }
    ...
    <div className="app-shell">
      ...
      <nav className="topbar">
        ...
        <div className="nav-actions">
          ...
          <button
            className={`demo-btn ${demoLoaded ? 'demo-loaded' : ''}`}
            onClick={loadDemo}
            disabled={submitting || demoLoaded}
          >
            {submitting ? (
              <LoaderCircle
                className="spin"
